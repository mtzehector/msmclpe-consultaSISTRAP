 ...
 */
@Provider
public class ConsultaPrestamosEnCursoPorMovimientoService extends ServiceDefinition<ConsultaDescuentosEmitidos, ConsultaDescuentosEmitidos> {
    ...
    public Message<ConsultaDescuentosEmitidos> execute(Message<ConsultaDescuentosEmitidos> request) throws BusinessException {
        log.log(Level.INFO, ">>>consultaSISTRAP|ConsultaPrestamosEnCursoPorMovimientoService|execute {0}", request.getPayload().getDescuentosRequest());
        Response response = null;
        try {
            response = client.consultaPrestamosconDescuento(request.getPayload().getDescuentosRequest());
        } catch (Exception e) {
            e.printStackTrace();
...
//                                    res,
//                                    endPoint+"sipre/ConsultaPrestamosconDescuento",
//                                    true,null
//                            )
//                    )
//            );
