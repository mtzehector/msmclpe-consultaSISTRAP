 ...
 */
@Provider
public class ConsultaDescuentosEmitidosService extends ServiceDefinition<ConsultaDescuentosEmitidos, ConsultaDescuentosEmitidos> {
    ...
    public Message<ConsultaDescuentosEmitidos> execute(Message<ConsultaDescuentosEmitidos> request) throws BusinessException {
        log.log(Level.INFO, ">>>consultaSISTRAP|ConsultaDescuentosEmitidosService|execute {0}", request.getPayload().getDescuentosRequest());
        Response response = null;
        try {
            response = client.consultaDescuentoEmitido(
    ...
    @Override
    protected Message<ConsultaDescuentosEmitidos> onOk(Response response, Message<ConsultaDescuentosEmitidos> request) {
        ConsultaDescuentoEmitidoResponse res = response.readEntity(ConsultaDescuentoEmitidoResponse.class);
        request.getPayload().setDescuentosResponse(res);
...
//                                    request.getPayload().getDescuentosRequest(),
//                                    res,
//                                    endPoint+"sipre/ConsultaDescuentoEmitido",
//                                    true,
//                                    null
//                            )
//                    )
...
//        }
        ...
        log.log(Level.INFO, ">>>consultaSISTRAP|ConsultaDescuentosEmitidosService|salida {0}", request.getPayload().getDescuentosResponse().getDescuentosEmitidos().size());
        return request;
    }
}
