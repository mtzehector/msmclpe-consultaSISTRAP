 ...
 */
@Provider
public class ConsultaEstadosSPESService extends ServiceDefinition<ConsultaEstadosSPES, ConsultaEstadosSPES> {
    ...
    @Override
    public Message<ConsultaEstadosSPES> execute(Message<ConsultaEstadosSPES> message) throws BusinessException {
        log.log(Level.INFO, "Sipre ConsultaPrestamo {0}", message.getPayload().getRequest());
        Response response = null;
        try {
            response = client.consultaEstadosSPES(message.getPayload().getRequest());
    ...
    @Override
    protected Message<ConsultaEstadosSPES> onOk(Response response, Message<ConsultaEstadosSPES> message) {
        ConsultaEstadosSPESResponse responseEntity = response.readEntity(ConsultaEstadosSPESResponse.class);
        message.getPayload().setResponse(responseEntity);
...
//                                    responseEntity,
//                                    endPoint + "sipre/ConsultaEstadosSPES",
//                                    true,null
//                            )
//                    )
//            );
